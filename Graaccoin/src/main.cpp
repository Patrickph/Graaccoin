...
#if defined(NDEBUG)
# error "Graaccoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x05c48c2e5043046e2effe855bdfdc0eff6b628314d399883d5713533cb37b465");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Graaccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Graaccoin Signed Message:\n";
...
{
    // Graaccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Graaccoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Graaccoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Graaccoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Graaccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Graaccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x55e70c1b89beea7fae029b58a0c60abe4994d955274e1d77a2a3d06fed7fb49a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Graaccoin helping to save lives";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1530903686;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085023572;

        if (fTestNet)
        {
            block.nTime    = 1530903658;
            block.nNonce   = 2085199892;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x8103bfb3f2d24e222681d5a4803cc068919bfd197af7cc27ff7fbb820c410180"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xc5, 0xb1, 0xd2 }; // Graaccoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GraaccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GraaccoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GraaccoinMiner(CWallet *pwallet)
{
    printf("GraaccoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("graaccoin-miner");
    ...
    try { loop {
        ...
        printf("Running GraaccoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GraaccoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GraaccoinMiner, pwallet));
}
